 ...
 */
 export const getTeams = async () => {
    return execute(TeamQueries.GetTeams, []);
  };
  ...
  export const getTeamById = async (id: ITeam['id']) => {
    return execute(TeamQueries.GetTeamsById, [id]);
  };
  ...
  export const insertTeam = async (team: ITeam) => {
    const result = await execute(TeamQueries.AddTeam, [
      team.name,
      team.league
    ]);
  ...
  export const updateTeam = async (team: ITeam) => {
    const result = await execute(TeamQueries.UpdateTeamById, [
      team.name,
      team.league,
      team.id
    ]);
   ...
   */
   export const deleteTeam = async (id: ITeam['id']) => {
    const result = await execute(TeamQueries.DeleteTeamById, [
      id
    ]);
    return result.affectedRows > 0;
  };